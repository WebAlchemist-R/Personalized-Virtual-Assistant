...
def summarize_content(messages):
    try:
        ...
        return f"Error occured in summarizing :{e}"
...
#the user functions but not using currently 
def news():
    api_key = os.getenv("NEWS_API_KEY")
    # Replace 'YOUR_API_KEY' with your actual NewsAPI key
    
    # Endpoint for fetching top headlines
    TOP_HEADLINES_URL = 'https://newsapi.org/v2/top-headlines'
    
    # Parameters for the request
    params = {
        'country': 'in',  # Change this to the country you're interested in
        'apiKey': api_key
    }
    ...
    if response.status_code == 200:
        ...
        # Extracting and printing the news articles
        articles = data['articles']
        for article in articles:
            title = article['title']
            source = article['source']['name']
            news_articles.append({'title': title, 'source': source})
            print(f"Title: {title}\nSource: {source}\n")
            return news_articles
    else:
        print("Error fetching news:", response.status_code)
        return f"Error fetching mews:{response.status_code}"
def google_search(query):
    top_results = list(googlesearch.search(query, num_results=5))
...
def get_name(name=None):
    if not name:
        ...
        if len(result) > 0 and not result[0].empty:
            ...
            print("Error: Unable to recognize the face. Please try again later. Your face is not clear.")
            return "Error: Unable to recognize the face. Please try again later. Your face is not clear."
...
def translate_to_detected_language(text, language_code):
    try: 
...
function_definitions=[
    ...
    {"name": "write_to_file",
        "description": "Writes content list  to a file .by using this function u can use it to store the history.",
        "parameters": {
            "type": "object",
            "properties": {
                "filename":{
                    "type": "string",
                    "description":"this is the file name histoty.txt"
                },
                "content": {
                    "type": "string",
     ...
     {"name": "read_from_file",
        ...
        "parameters": {
            ...
            "properties": {
                "filename":{
                    "type":"string",
                    "description":"the filename where you read the history."
                }
            }
        }
    },
    ...
    {"name":"translate_to_detected_language",
      "description":"this is the function which converts english to given language.you priovide the language using language code",
      "parameters":{
          "type":"object",
          "properties":
              {
