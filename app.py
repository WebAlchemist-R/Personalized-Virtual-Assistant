...
class Backend:
    ...
    def parse_function_response(self, message):
    ...
    def gemini(self):
        ...
        if(self.count_tokens(self.messages)<30720):
            ...
            if response.status_code != 200:
                ...
                if response.status_code==500:
                    return f"an iternal server has occured on google please try reloading the page.{response.text}"
                if response.status_code==400:
                    return f"an  internal server error please make sure the requsest is between user and model{response.text}"
                return f"unable to send a request to the gemini server please reload the page to try again{response.text}"
        else:
        ...
        if "content" not in response["candidates"][0]:
            ...
            return f"unable to  generated  the response.try again.the problem is{response} "
        ...
        if "functionCall" in message[0]:
            function_name, function_response = self.parse_function_response(message)
            message = {
                "role": "function",
                "parts": [
                    {
        ...
        with open('.env', 'r') as f:
            for line in f:
                ...
                if 'NEWS_API_KEY' in line:
                    news_key_found = True
            if google_key_found and news_key_found:
                return True
        return False
...
def save_keys():
    ...
    with open('.env', 'w') as f:
        ...
        f.write(f"""TF_ENABLE_ONEDNN_OPTS=0=0\n""")
